...
ギャラリーの4展示は**順路**として並んでいる。物語が 6 次元で言葉を使い切った直後に、
その続きから始まり、いちばん強い一枚絵で終わる。

1. **POLYTOPE EXPLORER** — n次元超立方体・単体・正軸体(n = 3…10)。**七次元から始まる** ── 物語が到達した 6 の、次の一歩
2. **PERSPECTIVE** — 選んだ次元の住人として別の次元を見る。断面・影・そして高次元からのX線俯瞰。「なぜ見えないのか」の答え
3. **CLIFFORD TORUS** — 4次元の二重回転により、自身を通り抜けて裏返るトーラス
4. **HOPF FIBRATION** — S³ を満たすファイバー円のステレオ投影。GPU上で4次元回転する数百本の光の輪。**3 の平らなトーラスは、この輪の族の一枚だった**
...
| **CTA「七次元から、はじめる」** | エピローグから、7次元超立方体(128頂点 / 448辺)の POLYTOPE EXPLORER へ。スクロール位置は保存され、いつでも戻れる。**タッチ端末では押す前から塗られている**(ホバーという予告が無い入力のため) |
| **上部タブ** | 4展示の切り替え(POLYTOPE / PERSPECTIVE / CLIFFORD / HOPF ── この並びが順路) |
...
表示中の展示は URL に載る。`?gallery=polytope` / `perspective` / `clifford` / `hopf` を
