...
/**
 * モバイルのボトムシート版レイアウトが有効になる条件(Phase 11)。
 *
 * **style.css のメディアクエリと 1 文字も違ってはならない** ── パネルの
 * --sheet-h 書き込み・ギャラリーのセーフエリア計算・CSS の三者が、
 * 同じ 1 本の条件で切り替わることがこのレイアウトの前提になっている。
 *
 * 除きたいのは「**横向きで背が低い**画面」(667×375 等)だけ。そこでシートを
 * 出すと開いた瞬間に画面の半分が操作卓になり、見出しごと作品が埋まる。
 * 一方で 320×568 のような**縦持ちで背が低い**端末はシートのままが正しい
 * (側面ドックにすると操作卓が幅 134px まで痩せる ── 実測)。
 * よって「高さが足りる」か「縦持ちである」かのどちらかを満たせばシート版とする。
 */
export const SHEET_LAYOUT_QUERY =
  '(max-width: 768px) and (min-height: 600px), (max-width: 768px) and (orientation: portrait)';
