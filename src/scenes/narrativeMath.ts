...
export const FRONT_SIGMA = 0.21;
/**
 * 前線から離れた場所へ残す下駄。
 *
 * **0.35 では読めなかった(Phase 28b)。** 芯の明るさは飽和が上限で、251〜252 で
 * 天井に当たっている ── そこから先に効く量は**面積**しかない。0.6 まで上げると
 * 暖色画素が e=0.70 で 2,644 → 3,838 に増え、しかも飽和画素は 0 のまま。
 * 「生まれつつある軸の辺ぜんたいが温まり、前線の芯だけがさらに明るい」という
 * 二段の読みになる。
 */
export const FRONT_BASE = 0.6;
/** 前線の先行。1 だと到達が包絡 0 と同時になり、着地が見えない */
export const FRONT_LEAD = 1.25;
/**
 * 前線の芯へ戻す明るさ(倍率 = 1 + BOOST·env·g)。
 *
 * **一度 0.9 にして、見えないと言われた(Phase 28b)。** 当時は「ゴールドの総量を
 * 旧実装より増やさない」を自分で縛りに置いていたが、既知の罠 #6 が禁じているのは
 * 総量ではなく **ベール(低周波のにじみ)と白飛び**である ── 実測では誕生の窓の飽和画素は
 * 3〜14 個で、6 次元の平常時(120 個)の 1 割にも届いていなかった。**予算は余っていた。**
 *
 * 2.4 は「芯が 3 倍以上明るくなり、なお飽和とベールが増えない」上限として実測で決めた値。
 * 一様に撒き直すのではなく**芯へ戻す**のがこの機能の趣旨なので、掛けるのはガウスの山だけ。
 */
export const FRONT_BOOST = 2.4;
