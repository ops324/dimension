 ...
 * 中身は 3 つの層でできている:
 *
 *   ① ドローン   49.50Hz と 49.85Hz の正弦を同じ強さで重ねる。差の 0.35Hz が
 *                 うなりになって、およそ 2.9 秒周期でゆっくり明滅する ──
 *                 これがこの空間の**呼吸**。99.6Hz(ほぼ 2 倍音)を弱く足して
 *                 芯を与え、198Hz のローパス(Q 0.5)で上を全部落とす。
 *
 *                 49.5Hz は **396Hz(ソルフェジオ「解放」= 3D の鐘)の 1/8**。
 *                 つまりこの空間の最低音は、私たちの次元の鐘を三オクターブ下ろした
 *                 ものである。ドローン・鐘・バイノーラルの三層が 99Hz の
 *                 ひとつの倍音列に乗り、そのどこを取ってもソルフェジオに重なる。
...
/** ドローンの 2 本。49.5 = 396Hz ÷ 8。差の 0.35Hz が「呼吸」になる */
const DRONE_A_HZ = 49.5;
const DRONE_B_HZ = 49.85;
const DRONE_GAIN = 0.5;
/** ほぼ 2 倍音。芯を与えるだけなので薄く */
const DRONE_C_HZ = 99.6;
const DRONE_C_GAIN = 0.18;
/** ドローンのローパス(倍音を全部落として「遠さ」を作る)。198 = 396Hz ÷ 2 */
const LOWPASS_HZ = 198;
const LOWPASS_Q = 0.5;
